...
def extract_data_from_remote_zip_folder(zip_url: str, path_to_folder: str) -> pd.DataFrame:
    ...
    with zipfile.ZipFile(io.BytesIO(r.content)) as z:
        ...
        file_paths = sorted([path for path in z.namelist() if path.startswith(path_to_folder)
                             if path.endswith(".csv")])
        # Read each file and concatenate them in a single dataframe.
        data = pd.concat(
            [pd.read_csv(z.open(file_name)) for file_name in file_paths], ignore_index=True)
